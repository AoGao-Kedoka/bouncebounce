...
void SPHSimulator::computeDensityAndPressure(Particle* p)
{
	...
	for (const Particle*  p_j_pointer : this->spatialHash->collisions(p)) {
		Particle p_j = *p_j_pointer;
...
void SPHSimulator::computeForces(Particle* p)
{
	...
	std::vector<const Particle*> neighbours = this->spatialHash->collisions(p);
	if (print) std::cout << "Neighbours: " << neighbours.size()  << "\t Instead of: "<< this->particles.size() << std::endl;

	for (const Particle* p_j_pointer : neighbours) {
		Particle p_j = *p_j_pointer;
		// TODO do this better
		// i == j 
...
std::vector<const Particle*> SpatialHash::collisions(const Particle*  p) {
	...
	for (auto x =-1; x <= 1; x++) {
		for (auto y = -1; y <= 1; y++) {
			for (auto z = -1; z <= 1; z++) {
				...
				std::vector<const Particle*> temp = (*this)[Vec3(p->pos.x + h * x,
					p->pos.y + h * y, 
					p->pos.z + h * z)];

				std::copy_if(temp.begin(), temp.end(), std::back_inserter(collisions),
					[&collisions](const Particle* particle) {return std::find(collisions.begin(), collisions.end(), particle) == collisions.end(); }
				);
...
std::vector<const Particle*>& SpatialHash::operator[](const Vec3& pos) {
